...
import { 
  ...
  Download, Share, Info, X, Loader // 🔴 เพิ่ม Import ไอคอนที่จำเป็น
} from "lucide-react";
...
export default function AdminShift() {
  ...
  const { language, shifts, deleteShift, divisions, sections, departments } = useAppStore();
  ...
  const ALL_COMPANY_DEPARTMENTS = useMemo(() => departments.map(d => d.name), [departments]);
  ...
  const groupedShiftsFlat = useMemo(() => {
    const filtered = shifts.filter(s => s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.department.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const getDisplayDate = (startDateStr: string, endDateStr: string) => {
    return startDateStr === endDateStr 
      ? new Date(startDateStr).toLocaleDateString(language === 'th' ? 'th-TH' : 'en-GB', { month: 'short', day: 'numeric', year: 'numeric' })
    ...
    <div className="p-6 md:p-8 space-y-6 min-h-screen bg-slate-50/50 pb-24 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-slate-500 font-medium text-sm mt-1">จัดการกะการทำงานตามโครงสร้างองค์กร (แอดมิน)</p>
        </div>
                                  ...
                                  <div className="bg-slate-50/50 border-t border-slate-100">
                                    {deptShifts.length === 0 ? (
                                      <div className="py-8 text-center border-b border-slate-100 bg-white">
                                        ...
                                        <button onClick={() => router.push(`/shift/create?dept=${dept}`)} className="text-[10px] font-black text-blue-600 bg-blue-50 px-4 py-2 rounded-lg hover:bg-blue-100 uppercase tracking-widest inline-flex items-center gap-1">สร้างกะให้แผนกนี้</button>
                                      </div>
                                    ) : (
                                        ...
                                        <table className="w-full text-left whitespace-nowrap bg-white">
                                          <thead className="bg-slate-100/50 border-b border-slate-200 text-[10px] uppercase font-black text-slate-400 tracking-widest">
                                            <tr>
                                              ...
                                              <th className="p-3 text-center">พนักงาน (Staff)</th>
                                              <th className="p-3 text-right pr-5">จัดการ (Action)</th>
                                            </tr>
                                          </thead>
                                          <tbody className="divide-y divide-slate-100 text-sm">
                                            {deptShifts.map(shift => (
                                              <tr key={shift.id} className="hover:bg-blue-50/30 transition-colors group">
                                                <td className="p-3 pl-5 font-black text-slate-800 text-sm">{shift.name}</td>
                                                <td className="p-3 text-[11px] font-bold text-slate-500"><CalendarDays size={12} className="inline text-blue-500 mr-1"/> {getDisplayDate(shift.startDate || shift.date, shift.endDate || shift.date)}</td>
                                                <td className="p-3 text-[11px] font-black text-slate-600"><Clock size={12} className="inline text-indigo-500 mr-1"/> {shift.startTime} - {shift.endTime} น.</td>
                                                <td className="p-3 text-center"><span className="bg-blue-50 text-blue-600 font-black text-[10px] px-2 py-1 rounded-lg border border-blue-100"><Users size={10} className="inline mr-1"/> {shift.employeeIds?.length || 0} คน</span></td>
                                                <td className="p-3 text-right pr-5">
                                                  <div className="flex justify-end gap-2">
                                                    ...
                                                    <button onClick={() => setDeleteConfirm(shift.id)} className="p-1.5 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-lg transition-all"><Trash2 size={14}/></button>
                                                  </div>
                                                </td>
                                              </tr>
                                            ))}
                  ...
                  <div className="border-t border-slate-100 bg-white animate-in slide-in-from-top-2 fade-in duration-200">
                    {deptShifts.length === 0 ? (
                      <div className="p-8 text-center border-b border-slate-100">
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">ยังไม่มีกะในแผนกนี้</p>
                        <button onClick={() => router.push(`/shift/create?dept=${dept}`)} className="text-[10px] font-black text-blue-600 bg-blue-50 px-4 py-2 rounded-lg hover:bg-blue-100 uppercase tracking-widest inline-flex items-center gap-1">สร้างกะให้แผนกนี้</button>
                      </div>
                    ) : (
                      <div className="overflow-x-auto w-full">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center animate-in zoom-in-95">
            ...
            <p className="text-sm font-bold text-slate-500 mb-8">คุณแน่ใจหรือไม่ว่าต้องการลบกะการทำงานนี้?</p>
