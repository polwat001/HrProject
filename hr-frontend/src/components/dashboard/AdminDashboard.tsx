...
import {
  ...
  FileText, Edit2, Trash2, Plus, X, Loader
} from "lucide-react";
...
import { MOCK_EMPLOYEES, MOCK_ATTENDANCE, MOCK_LEAVES, MOCK_OTS, todayStr } from "@/mocks/dashboardData";
    ...
    <div className="p-8 space-y-8 bg-slate-50/50 min-h-screen animate-in fade-in duration-500">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <AdminStatCard title="Total OT Cost (ค่าล่วงเวลารวม)" value={`฿${financialStats.totalOtCost.toLocaleString()}`} icon={Clock} color={financialStats.otExceed ? "red" : "green"} highlightValue={financialStats.otExceed} />
          ...
          <Card className="rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between p-6 pb-2">
              ...
              <select value={attendanceTimeframe} onChange={(e) => setAttendanceTimeframe(e.target.value)} className="text-base font-bold text-slate-500 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 outline-none cursor-pointer hover:bg-slate-100 focus:ring-2 focus:ring-blue-500">
                <option value="daily">รายวัน (Daily)</option><option value="weekly">สัปดาห์นี้ (Weekly)</option><option value="monthly">เดือนนี้ (Monthly)</option><option value="quarterly">ไตรมาสนี้ (Quarterly)</option><option value="yearly">ปีนี้ (Yearly)</option>
              </select>
            </CardHeader>
          ...
          <Card className="rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <div className="relative flex-1 sm:flex-none">
                  <select value={leaveDepartment} onChange={(e) => setLeaveDepartment(e.target.value)} className="w-full text-xs font-bold text-slate-500 bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-3 py-2 outline-none cursor-pointer hover:bg-slate-100 focus:ring-2 focus:ring-blue-500 appearance-none">
                    <option value="all">ทุกแผนก (All)</option><option value="IT">IT</option><option value="Marketing">Marketing</option><option value="Human Resources">HR</option><option value="Finance">Finance</option><option value="Operations">Operations</option>
                  </select>
                </div>
                <div className="relative flex-1 sm:flex-none">
            ...
            <CardContent className="px-6 pt-6 pb-6">
              <div className="h-[200px] w-full">
                <Bar 
                  data={{ labels: displayLeaveBreakdown.map(l => l.name), datasets: [{ data: displayLeaveBreakdown.map(l => l.value), backgroundColor: displayLeaveBreakdown.map(l => l.color), borderRadius: 6, barThickness: 24 }] }}
            ...
            <CardContent className="p-0">
              <table className="w-full text-left text-sm">
                <thead className="bg-white border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr>
                    <th className="py-4 px-6 text-center">Rank</th><th className="py-4 px-6">Department</th><th className="py-4 px-6 text-right">Total Cost (THB)</th><th className="py-4 px-6 text-right">Trend</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {topCostCenters.map((item) => (
                    <tr key={item.rank} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-4 px-6"><p className="font-bold text-slate-800">{item.dept}</p><p className="text-[10px] font-bold text-slate-400 mt-0.5">Head: {item.headName}</p></td>
                      <td className="py-4 px-6 text-right font-mono font-bold text-slate-700">{item.cost.toLocaleString()}</td>
                      <td className="py-4 px-6 text-right"><span className={`inline-flex items-center px-2 py-1 rounded text-[10px] font-black ${item.trend.startsWith('+') ? 'text-red-600 bg-red-50' : item.trend.startsWith('-') ? 'text-green-600 bg-green-50' : 'text-slate-500 bg-slate-100'}`}>{item.trend}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Card className="rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col h-[600px]">
            <CardTitle className="text-xl font-black mb-6 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 uppercase tracking-tight">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleOpenAddHoliday} className="flex items-center gap-1 text-base font-black uppercase tracking-widest bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-colors px-3 py-1.5 rounded-xl border border-red-100"><Plus size={14} /> Add Holiday</button>
            ...
            <div className="flex flex-col lg:flex-row gap-6 h-full">
              <div className="w-full lg:w-1/3 flex flex-col border-b lg:border-b-0 lg:border-r border-slate-100 pb-4 pt-10 lg:pb-0 lg:pr-6">
                <h5 className="font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2"><FileText size={14} /> วันหยุดประจำเดือนนี้</h5>
                <div className="space-y-3 h-auto overflow-y-auto custom-scrollbar pr-2 flex-1">
                  {currentMonthHolidays.length === 0 ? (
                    ...
                    currentMonthHolidays.map((h) => {
                      ...
                      return (
                        <div key={h.id} className="flex gap-2 bg-slate-50 px-3 py-2.5 justify-between rounded-xl border border-slate-100 group hover:border-red-200 transition-all items-center">
                          <div className="flex items-center gap-3">
                            <span className="min-w-8 px-2 shrink-0 text-center text-base font-black text-red-500 bg-red-100 rounded-md py-0.5">{displayDate}</span>
                          ...
                          <div className="flex self-end gap-1 opacity-50 group-hover:opacity-100 transition-all">
                            <button onClick={() => handleEditHoliday(h)} className="p-1 text-slate-400 hover:text-blue-600 hover:bg-white rounded transition-colors" title="แก้ไข"><Edit2 size={16} /></button>
                            <button onClick={() => handleDeleteHoliday(h.id)} className="p-1 text-slate-400 hover:text-red-600 hover:bg-white rounded transition-colors" title="ลบ"><Trash2 size={16} /></button>
                          </div>
                        </div>
                      )
                    })
              ...
              <div className="w-full lg:w-2/3">
                <div className="grid grid-cols-7 gap-2">
                  {["จ", "อ", "พ", "พฤ", "ศ", "ส", "อา"].map((d) => (
                    <div key={d} className={`text-center text-base font-black uppercase tracking-widest mb-2 ${d === 'ส' || d === 'อา' ? 'text-red-400' : 'text-slate-400'}`}>{d}</div>
                  ))}
                  {adminCalendarData.map((cell, idx) => (
                    <div key={idx} title={cell.holiday?.title} className={`h-14 rounded-xl flex flex-col items-center justify-center border border-slate-200 relative ${cell.day === 0 ? "bg-transparent border-none" : "bg-white shadow-sm hover:border-blue-200 transition-colors cursor-pointer"} ${cell.holiday ? "bg-red-50 border-red-100" : ""}`}>
                      <span className={`text-xs font-black z-10 ${cell.isToday ? "text-white bg-blue-600 w-6 h-6 rounded-full flex items-center justify-center shadow-md" : cell.holiday ? "text-red-600" : "text-slate-600"}`}>{cell.day || ""}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-lg font-bold text-slate-800">{editingHolidayId ? "แก้ไขวันหยุด (Edit Holiday)" : "ตั้งค่าวันหยุด (Add Holiday)"}</h3>
              <button onClick={() => setShowHolidayModal(false)} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={20} /></button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">ชื่อวันหยุด (Title) <span className="text-red-500">*</span></label>
                <input type="text" value={holidayForm.title} onChange={(e) => setHolidayForm({ ...holidayForm, title: e.target.value })} placeholder="เช่น วันสงกรานต์" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">เริ่ม (Start) <span className="text-red-500">*</span></label>
                  <input type="date" value={holidayForm.startDate} onChange={(e) => setHolidayForm({ ...holidayForm, startDate: e.target.value })} className="w-full border border-slate-300 rounded-lg px-2 py-2 text-sm outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">สิ้นสุด (End)</label>
                  <input type="date" value={holidayForm.endDate} min={holidayForm.startDate} onChange={(e) => setHolidayForm({ ...holidayForm, endDate: e.target.value })} className="w-full border border-slate-300 rounded-lg px-2 py-2 text-sm outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" />
                </div>
              </div>
              <p className="text-[10px] text-slate-400 font-medium ">* หากเป็นวันหยุดแค่วันเดียว ไม่จำเป็นต้องใส่วันที่สิ้นสุด</p>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50">
              <button onClick={() => setShowHolidayModal(false)} className="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-200 rounded-lg transition-colors">ยกเลิก</button>
              <button onClick={handleSaveHoliday} className="flex items-center gap-2 px-5 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">{editingHolidayId ? "บันทึกการแก้ไข" : "บันทึกวันหยุด"}</button>
            </div>
          </div>
        </div>
      )}
